...
3. **Dead code** — unused functions, commented-out blocks, unreachable branches.
4. **Tangled logic** — business logic (NC parsing, FTP sync rules, ATC tool validation) embedded in API route handlers or React components that should be extracted into testable modules.
5. **Type safety gaps**:
...
- Don't propose refactors for code that has no test coverage — use `/jgl-review-testing` first.

## Refactor Threshold

- **No refactor needed** if: no files over 250 lines, no logic duplication beyond 2x, no tangled business logic, audits clean, type coverage solid. Say so and stop.
- **Refactor recommended** if any of these are true:
  - 3+ files over 250 lines
  - Same logic block duplicated 3+ times
  - Business logic embedded in route handlers or React components that can't be unit tested
