...
**Location:** [main.py:70-76](../backend/app/main.py#L70-L76)

**Startup Flow:**
1. Application initialized by uvicorn/gunicorn
2. FastAPI runs startup event handlers
3. PollingService.start() launches background polling loop
4. Polling begins for all enabled machines in database
5. API becomes ready to accept requests

**Important:** The polling service starts BEFORE the first HTTP request arrives. This ensures real-time data is available immediately.
...
**Location:** [main.py:79-85](../backend/app/main.py#L79-L85)
