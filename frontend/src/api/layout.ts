...
import { layoutConfigToPanes, layoutConfigToCompressorPanes, panesToLayoutConfig } from '../utils/layoutUtils';
 ...
 */
export async function fetchCompressorLayout(compressorId: number): Promise<PaneLayout[]> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/compressors/${compressorId}/layout`);
    if (!response.ok) {
      throw new Error(`Failed to fetch compressor layout: ${response.statusText}`);
    }
    const data = await response.json();
    return layoutConfigToCompressorPanes(data.layout_config);
  } catch (error) {
    console.error('Error fetching compressor layout:', error);
    return layoutConfigToCompressorPanes(null);
  }
}

export async function saveCompressorLayout(
  compressorId: number,
  panes: PaneLayout[],
  gridCols: number = 12
): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to save compressor layout: ${response.statusText} - ${errorText}`);
  }
}
