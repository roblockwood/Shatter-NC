...
function buildEnv(cfg) {
  const lines = [];
  lines.push("# Shatter runtime configuration");
  lines.push("# Generated locally. Do not commit this file.");
  lines.push("");
  lines.push("COMPOSE_PROJECT_NAME=shatter");
  lines.push("GITHUB_OWNER=roblockwood");
  lines.push("GITHUB_REPO=shatter-nc");
  lines.push("IMAGE_TAG=latest");
  lines.push("");
  lines.push(`POSTGRES_PASSWORD=${cfg.POSTGRES_PASSWORD}`);
  lines.push(`SECRET_KEY=${cfg.SECRET_KEY}`);
  lines.push(`CORS_ORIGINS=${cfg.CORS_ORIGINS}`);
  if (cfg.MQTT_ENABLED) {
    lines.push("");
...
function buildCompose(cfg) {
  ...
  backend:
    image: ghcr.io/\${GITHUB_OWNER:-roblockwood}/\${GITHUB_REPO:-shatter-nc}/backend:\${IMAGE_TAG:-latest}
    ...
    environment:
      ...
      CORS_ORIGINS: \${CORS_ORIGINS:-'["http://localhost"]'}${mqttBackendEnv}
    volumes:
  ...
  frontend:
    image: ghcr.io/\${GITHUB_OWNER:-roblockwood}/\${GITHUB_REPO:-shatter-nc}/frontend:\${IMAGE_TAG:-latest}
